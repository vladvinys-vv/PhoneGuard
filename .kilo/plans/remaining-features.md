...
## CallBlocker
- **SMS Blocking**: Android does not provide a public API for SMS blocking without carrier integration. Consider `SmsRetriever` / `CarrierMessagingService` or document the limitation.
...
- **Traffic Statistics**: Daily/weekly blocking statistics per app.
...
- **Scan Comparison**: Compare current scan results with previous scans.
...
- **Failed Attempts Photo**: Implemented via `FLAG_SECURE` and camera permission check.

## Performance
- **Camera Resolution**: Reduce `ShoulderSurferService` camera resolution for better battery life.
- **Network Caching**: Implement 24h cache for `getInstalledApps()` (done via `InstalledAppsAppsCache`).
